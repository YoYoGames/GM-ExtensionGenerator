...
public:
    ...
    // 1) Trivially-copyable read (excludes string_view-constructible types)
    template<class T, class Q = typename Span::element_type>
    std::enable_if_t<
        std::is_const_v<Q> && std::is_trivially_copyable_v<T> && !std::is_constructible_v<T, std::string_view>,
        T>
    read()
    {
        need(sizeof(T));
...
struct IByteWriter {
    ...
    {
        static_assert(std::is_trivially_copyable_v<T>, "IByteWriter::write only for trivially copyable types");
