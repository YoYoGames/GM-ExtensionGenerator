...
namespace extgen.Parsing.Gmidl
{
    ...
    internal partial class GmidlSchemaParser(GMIDLDatabase db)
    {
        ...
        {
            // Strip prefix/suffix from GMIDL class names (cls.Name[2..^2])
            // Guard against short names to avoid index errors.
        ...
        private IrType ResolveNamed(string name)
        {
            ...
            // Could throw earlier for stricter validation,
            // but keeping "unknown named type" visible is often useful during development.
            throw new InvalidOperationException($"Unknown named type '{name}'. Was it declared in the schema?");
        }
